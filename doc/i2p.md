# I2P support in Bitcoin Deis

It is possible to run Bitcoin Deis as an
...
## Run Bitcoin Deis with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://geti2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with Bitcoin Deis:
...
I2P support was added to Bitcoin Deis in version 22.0 and there may be fewer I2P
...
The first time Bitcoin Deis connects to the I2P router, it automatically
...
`i2p_private_key` in the Bitcoin Deis data directory.  The persistent I2P
...
## Fetching I2P-related information from Bitcoin Deis

There are several ways to see your I2P address in Bitcoin Deis if accepting
...
Bitcoin Deis uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and Bitcoin Deis
...
To allow future upgrades to newer versions of SAM, Bitcoin Deis sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Bitcoin Deis.
