Bitcoin Deis version 0.18.0 is now available from:
...
recommended to use Bitcoin Deis on unsupported systems.

Bitcoin Deis should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Bitcoin Deis does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Bitcoin Deis
...
`~bitcoin/.bitcoin`. This change makes Bitcoin Deis more consistent with
...
- A new document introduces Bitcoin Deis's BIP174 [Partially-Signed
...
- A new `--disable-bip70` option may be passed to `./configure` to
  ...
  Bitcoin Deis to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bitcoin Deis knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bitcoin Deis, subsequently importing
...
This section describes planned changes to Bitcoin Deis that may affect
other Bitcoin software and services.

- Since version 0.16.0, Bitcoin Deis’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bitcoin Deis 0.20 (expected about
  a year after 0.18), Bitcoin Deis will default to native segwit
  ...
  to bech32 addresses, and if the Bitcoin Deis project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bitcoin Deis 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bitcoin Deis
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bitcoin Deis
...
- A sub-project of Bitcoin Deis now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bitcoin Deis.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bitcoin Deis's own implementation, although entropy
  gathered by Bitcoin Deis is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bitcoin Deis a
...
- On macOS, Bitcoin Deis now opts out of application CPU throttling
...
- #11526 Visual Studio build configuration for Bitcoin Deis (sipsorcery)
...
- #12775 Integration of property based testing into Bitcoin Deis (Christewart)
