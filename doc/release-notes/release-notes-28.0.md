Bitcoin Deis version 28.0 is now available from:
...
Running Bitcoin Deis binaries on macOS requires self signing.
```
...
to `C:\Users\Username\AppData\Local\Bitcoin`. Bitcoin Deis will check the existence
...
- Previously if Bitcoin Deis was listening for P2P connections, either using
...
- Bitcoin Deis will now fail to start up if any of its P2P binds fail, rather
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Bitcoin Deis with the configuration
...
are now required to compile Bitcoin Deis. (#29091, #30263)

- The minimum required glibc to run Bitcoin Deis is now
...
Previous releases of Bitcoin Deis or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
