...
Bitcoin Deis version 0.21.0 is now available from:
...
From Bitcoin Deis 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Bitcoin Deis does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Bitcoin Deis, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Bitcoin Deis will no longer
...
- The `startupnotify` option is used to specify a command to
  execute when Bitcoin Deis has finished with its startup
  sequence. (#15367)
...
Bitcoin Deis will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Bitcoin Deis will now just log warnings instead of creating new wallets with
...
Users who already have a Bitcoin Deis wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Bitcoin Deis.
...
The change to using descriptors changes the default derivation paths used by Bitcoin Deis
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Bitcoin Deis is
...
- #18739 Document how to fuzz Bitcoin Deis using Honggfuzz (practicalswift)
