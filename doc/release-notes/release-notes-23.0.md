...
Bitcoin Deis version 23.0 is now available from:
...
- Before this release, Bitcoin Deis had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Bitcoin nodes listening on non-standard ports would likely not get any Bitcoin Deis peers connecting to them. This preference has been removed. (#23542)
...
Bitcoin Deis release binaries for Linux now include experimental tracepoints which
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Bitcoin Deis
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Bitcoin Deis in 22.0. (#22544)
