...
Currently, it is possible to create a multisig wallet using Bitcoin Deis only.
...
At the time of writing, there is no way to extract a specific path from wallets in Bitcoin Deis. For this, an external signer/xpub can be used.
...
Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Bitcoin Deis single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Bitcoin Deis will include the checksum in their output.
