# User-space, Statically Defined Tracing (USDT) for Bitcoin Deis

Bitcoin Deis includes statically defined tracepoints to allow for more
...
**Note**: Bitcoin Deis uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bitcoin Deis
...
To list probes in Bitcoin Deis, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Bitcoin Deis.
